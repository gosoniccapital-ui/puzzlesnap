...
export default function ThemeToggle({ className = "", showLabel = false }: ThemeToggleProps) {
  ...
  if (!isLoaded) {
    ...
    return (
      <div className={`w-9 h-9 rounded-xl border border-stone-250 dark:border-stone-800 bg-stone-100/50 dark:bg-stone-900/50 ${className}`} />
    );
  }
  ...
  return (
    <button
      onClick={toggleTheme}
      type="button"
      className={`relative group flex items-center justify-center gap-2 min-h-[44px] min-w-[44px] p-2.5 rounded-xl transition-all duration-200 cursor-pointer border ${
        isDark
